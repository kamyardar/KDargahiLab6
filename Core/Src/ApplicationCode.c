...
//		uint32_t ts = HAL_GetTick();
		startTetris();
//		uint32_t te = HAL_GetTick();
//		uint32_t dur = GetDuration(ts,te);
//		EndScreen(dur);
...
//	DisableInterruptIRQ(EXTI0_IRQ); // disable so no more interrupts triggered
//	toggleGreenLED();
//	ClearEXTIPending(0);
//	EnableInterruptIRQ(EXTI0_IRQ);
}
